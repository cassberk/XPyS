...
class VAMASExperiment:
    ...
    def read_from_file(self):
        ...
        self.format_identifier = lines.__next__().strip()
        self.institution_identifier = lines.__next__().strip()
        self.instrument_model_identifier = lines.__next__().strip()
        self.operator_identifier = lines.__next__().strip()
        self.experiment_identifier = lines.__next__().strip()
        self.number_of_lines_in_comment = int(lines.__next__().strip())

        self.comment = ""
        for i in range(self.number_of_lines_in_comment):
            self.comment = self.comment + lines.__next__()

        self.experiment_mode = lines.__next__().strip()
        self.scan_mode = lines.__next__().strip()
        ...
        if (self.experiment_mode.upper() == 'MAP' or
            ...
            self.number_of_spectral_regions = int(lines.__next__().strip())

        if (self.experiment_mode.upper() == 'MAP' or
            self.experiment_mode.upper() == 'MAPDP'):
            self.number_of_analysis_positions = int(lines.__next__().strip())
            self.number_of_discrete_x_coordinates_available_in_full_map = int(lines.__next__().strip())
            self.number_of_discrete_y_coordinates_available_in_full_map = int(lines.__next__().strip())

        self.number_of_experimental_variables = int(lines.__next__().strip())

        for i in range(self.number_of_experimental_variables):
            self.experimental_variable_labels.append(lines.__next__().strip())
            self.experimental_variable_units.append(lines.__next__().strip())

        self.number_of_entries_in_parameter_inclusion_or_exclusion_list = int(lines.__next__().strip())

        for i in range(abs(self.number_of_entries_in_parameter_inclusion_or_exclusion_list)):
            self.parameter_inclusion_or_exclusion_prefix_numbers.append(int(lines.__next__().strip()))

        self.number_of_manually_entered_items_in_block = int(lines.__next__().strip())

        for i in range(self.number_of_manually_entered_items_in_block):
            self.prefix_numbers_of_manually_entered_items.append(int(lines.__next__().strip()))

        self.number_of_future_upgrade_experiment_entries = int(lines.__next__().strip())

        self.number_of_future_upgrade_block_entries = int(lines.__next__().strip())

        for i in range(self.number_of_future_upgrade_block_entries):
            self.future_upgrade_experiment_entries.append(lines.__next__().strip())

        self.number_of_blocks = int(lines.__next__().strip())

        for i in range(self.number_of_blocks):
            block = VAMASBlock()
            block.block_identifier = lines.__next__().strip()
            block.sample_identifier = lines.__next__().strip()
            # 1
            year = int(lines.__next__().strip())

            # 2
            month = int(lines.__next__().strip())

            # 3
            day = int(lines.__next__().strip())

            # 4
            hours = int(lines.__next__().strip())
            if hours == 24:
                hours = 0 #catch a 24:00 hours ambiguity

            # 5
            minutes = int(lines.__next__().strip())

            # 6
            seconds = int(lines.__next__().strip())
            try:
            ...
            # 7
            block.number_of_hours_in_advance_of_greenwich_mean_time = int(lines.__next__().strip())

            # 8
            block.number_of_lines_in_block_comment = int(lines.__next__().strip())
            for i in range(block.number_of_lines_in_block_comment):
                block.comment = block.comment + lines.__next__()

            # 9
            block.technique = lines.__next__().strip()

            # 10
            if (self.experiment_mode.upper() == 'MAP' or
                self.experiment_mode.upper() == 'MAPDP'):
                block.x_coordinate = int(lines.__next__().strip())
                block.y_coordinate = int(lines.__next__().strip())

            # 11
            for i in range(self.number_of_experimental_variables):
                block.values_of_experimental_variables.append(float(lines.__next__().strip()))

            # 12
            block.analysis_source_label = lines.__next__().strip()
            ...
            if (self.experiment_mode.upper() == 'MAPDP' or
                ...
                block.sputtering_ion_or_atomic_number = lines.__next__().strip()
                block.number_of_atoms_in_sputtering_ion_or_atom_particle = int(lines.__next__().strip())
                block.sputtering_ion_or_atom_charge_sign_and_number = int(lines.__next__().strip())

            # 14
            block.analysis_source_characteristic_energy = float(lines.__next__().strip())

            # 15
            block.analysis_source_strength = float(lines.__next__().strip())

            # 16
            block.analysis_source_beam_width_x = float(lines.__next__().strip())
            block.analysis_source_beam_width_y = float(lines.__next__().strip())
            ...
            if (self.experiment_mode.upper() == 'MAP' or
                ...
                block.field_of_view_x = float(lines.__next__().strip())
                block.field_of_view_y = float(lines.__next__().strip())

            # 18
            if (self.experiment_mode.upper() == 'MAPSV' or
                self.experiment_mode.upper() == 'MAPSVDP' or
                self.experiment_mode.upper() == 'SEM'):
                block.first_linescan_start_x_coordinate = int(lines.__next__().strip())
                block.first_linescan_start_y_coordinate = int(lines.__next__().strip())
                block.first_linescan_finish_x_coordinate = int(lines.__next__().strip())
                block.first_linescan_finish_y_coordinate = int(lines.__next__().strip())
                block.last_linescan_finish_x_coordinate = int(lines.__next__().strip())
                block.last_linescan_finish_y_coordinate = int(lines.__next__().strip())

            # 19
            block.analysis_source_polar_angle_of_incidence = float(lines.__next__().strip())

            # 20
            block.analysis_source_azimuth = float(lines.__next__().strip())

            # 21
            block.analyser_mode = lines.__next__().strip()

            # 22
            block.analyser_pass_energy_of_retard_ratio_or_mass_resolution = float(lines.__next__().strip())

            # 23
            if (block.technique.upper() == 'AES DIFF'):
                block.differential_width = float(lines.__next__().strip())

            # 24
            block.magnification_of_analyser_transfer_lens = float(lines.__next__().strip())

            # 25
            block.analyser_work_function_or_acceptance_energy_of_atom_or_ion = float(lines.__next__().strip())

            # 26
            block.target_bias = float(lines.__next__().strip())

            # 27
            block.analysis_width_x = float(lines.__next__().strip())
            block.analysis_width_y = float(lines.__next__().strip())

            # 28
            block.analyser_axis_take_off_polar_angle = float(lines.__next__().strip())
            block.analyser_axis_take_off_azimuth = float(lines.__next__().strip())

            # 29
            block.species_label = lines.__next__().strip()

            # 30
            block.transition_or_charge_state_label = lines.__next__().strip()
            block.charge_of_detected_particle = int(lines.__next__().strip())

            # 31
            if (self.scan_mode.upper() == 'REGULAR'):
                block.abscissa_label = lines.__next__().strip()
                block.abscissa_units = lines.__next__().strip()
                block.abscissa_start = float(lines.__next__().strip())
                block.abscissa_increment = float(lines.__next__().strip())

            # 32
            block.number_of_corresponding_variables = int(lines.__next__().strip())
            for i in range(block.number_of_corresponding_variables):
                block.corresponding_variable_labels.append(lines.__next__().strip())
                block.corresponding_variable_units.append(lines.__next__().strip())

            # 33
            block.signal_mode = lines.__next__().strip()

            # 34
            block.signal_collection_time = float(lines.__next__().strip())

            # 35
            block.number_of_scans_to_compile_this_block = int(lines.__next__().strip())

            # 36
            block.signal_time_correction = float(lines.__next__().strip())
            ...
            if ( (block.technique.upper() == 'AES DIFF' or
                ...
                block.sputtering_source_energy = float(lines.__next__().strip())
                block.sputtering_source_beam_current = float(lines.__next__().strip())
                block.sputtering_source_width_x = float(lines.__next__().strip())
                block.sputtering_source_width_y = float(lines.__next__().strip())
                block.sputtering_source_polar_angle_of_incidence = float(lines.__next__().strip())
                block.sputtering_source_azimuth = float(lines.__next__().strip())
                block.sputtering_mode = lines.__next__().strip()

            # 38
            block.sample_normal_polar_angle_of_tilt = float(lines.__next__().strip())
            block.sample_normal_tilt_azimuth = float(lines.__next__().strip())

            # 39
            block.sample_rotation_angle = float(lines.__next__().strip())

            # 40
            block.number_of_additional_numerical_parameters = int(lines.__next__().strip())
            for i in range(block.number_of_additional_numerical_parameters):
                block.additional_numerical_parameter_labels.append(lines.__next__().strip())
                block.additional_numerical_parameter_units.append(lines.__next__().strip())
                block.additional_numerical_parameter_values.append(float(lines.__next__().strip()))

            # end numbered lines in block specification
            for i in range(self.number_of_future_upgrade_block_entries):
                block.future_block_entries.append(lines.__next__().strip())

            block.number_of_ordinate_values = int(lines.__next__().strip())

            variables = []
            for i in range(block.number_of_corresponding_variables):
                block.minimum_ordinate_values.append(float(lines.__next__().strip()))
                block.maximum_ordinate_values.append(float(lines.__next__().strip()))
                variables.append([])
            ...
            for i in range(int(block.number_of_ordinate_values/block.number_of_corresponding_variables)):
                for j in range(block.number_of_corresponding_variables):
                    variables[j].append(float(lines.__next__().strip()))
