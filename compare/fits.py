...
def fit_spectogram(input_samples,sputter_time = None,specify_scans = None, sigma_weight = 1,c_weight = 0.5,multiple_samples = False,\
    axis = 0,pltlims = 0,fig = False, ax = False):
    ...
    for sample_name,sample in input_samples.items():
        for scan in spectra_list:
            ...
            for pairs in spec.pairlist:
                df[sample_name+'_'+spec.orbital+'_'+pairs[0]+'amp'] = pd.Series([sum([spec.fit_results[i].params[p+'amplitude'].value for p in pairs]) for i in range(len(spec.fit_results))])
                df[sample_name+'_'+spec.orbital+'_'+pairs[0]+'cent'] = pd.Series([spec.fit_results[i].params[pairs[0]+'center'].value  for i in range(len(spec.fit_results))])
                df[sample_name+'_'+spec.orbital+'_'+pairs[0]+'sig'] = pd.Series([spec.fit_results[i].params[pairs[0]+'sigma'].value  for i in range(len(spec.fit_results))])
    ...
    for sample_name,sample in input_samples.items():
        ...
        for scan in orderlist[::-1]:
            ...
            if sputter_time == None:
                ...
                if pltlims > sputter_time*len(spec.isub):
                    yrange = int(np.ceil(pltlims/sputter_time))
                    e, d = np.meshgrid(np.linspace(np.min(spec.esub), np.max(spec.esub), len(spec.esub)), np.linspace(0, yrange*sputter_time, yrange))
                else:
                    e, d = np.meshgrid(np.linspace(np.min(spec.esub), np.max(spec.esub), len(spec.esub)), np.linspace(0, sputter_time*len(spec.isub), len(spec.isub)))
                    

            c = np.zeros(e.shape)
            amplist = [k for k in [i for i in df.keys() if scan in i] if ('amp' in k)]
            maxamp = np.max([np.array(df[amplist[i]][df[amplist[i]].notna()]) for i in range(len(amplist))])
            for pairs in spec.pairlist:
                specpars = [k for k in [i for i in df.keys() if scan in i] if (pairs[0] in k) and (sample_name in k)]

                for i in range(len(spec.isub)):
                    specmap = df[specpars[0]][i]*gaussian(e[i,:],amplitude = 1,center = df[specpars[1]][i],sigma = df[specpars[2]][i]*sigma_weight)
                    c[i,:] += specmap
                    c[i,:] += specmap/maxamp

            # print(scan,pairs,c.max(),maxamp)
            ...
            axes[j].get_yaxis().set_ticks(np.arange(len(spec.isub)))
            ...
            if multiple_samples:
                ...
                if samplenum == 0:
                    axes[j].set_title(scan,fontsize = 30)
