    ...
    for sample in enumerate(sample_list):
        ...
        for spectra in enumerate(spectra_list):
            ...
            if spectra[1] not in fig_legend:
                ...
                fig_list.append(ax.bar(sample[0]+spectra[0]*spectra_width, at_pct, spectra_width, yerr=at_pct_err,\
                              error_kw=dict(lw=5, capsize=20, capthick=3), color = spectra_colors[spectra[1]]))
                
            else:
                ax.bar(sample[0]+spectra[0]*spectra_width, at_pct, spectra_width, yerr=at_pct_err,\
                              error_kw=dict(lw=5, capsize=20, capthick=3), color = spectra_colors[spectra[1]])
